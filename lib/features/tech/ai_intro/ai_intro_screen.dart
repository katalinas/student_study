/// AI 科普知识卡片页面。
///
/// 从内容加载器读取 AI 相关科普卡片，以可展开的知识卡片形式
/// 展示标题、摘要和详细内容，帮助学生了解人工智能基础知识。
library;
...
/// AI 科普卡片的内容加载提供者。
final _aiIntroCardsProvider =
    FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final loader = ContentLoader();
  return loader.loadCards('tech', category: 'ai');
});
...
const _themeColor = Color(0xFF26A69A);

// ---------------------------------------------------------------------------
// 页面组件
// ---------------------------------------------------------------------------

/// AI 科普知识卡片页面，以可展开列表展示 AI 相关知识。
class AiIntroScreen extends ConsumerWidget {
  const AiIntroScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final cardsAsync = ref.watch(_aiIntroCardsProvider);
    final theme = Theme.of(context);

    return Scaffold(
      appBar: AppBar(
        title: const Text('AI 科普'),
