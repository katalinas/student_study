...
/// 空间推理题目的内容加载提供者。
final _spatialQuestionsProvider = FutureProvider<List<Question>>((ref) async {
  final loader = ContentLoader();
  return loader.loadQuestions('logic', tag: 'spatial');
});
...
/// 空间站模块的主题色。
const _themeColor = Color(0xFF42A5F5);

/// 主题色渐变的第二色。
const _themeColorLight = Color(0xFF64B5F6);
...
/// 空间站主页面，展示关卡选择网格和专项训练入口。
class SpatialScreen extends ConsumerStatefulWidget {
  const SpatialScreen({super.key});

  @override
  ConsumerState<SpatialScreen> createState() => _SpatialScreenState();
}

class _SpatialScreenState extends ConsumerState<SpatialScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final questionsAsync = ref.watch(_spatialQuestionsProvider);
  ...
  void _startLevel(int level, _LevelStatus status) {
    ...
    final questionsAsync = ref.read(_spatialQuestionsProvider);
